...
import { SubscriptionSchema, InstallmentPlanSchema, LoanSchema, OwnershipTransferSchema } from '../schemas';
import { logAudit } from '../utils';
const planning = new Hono();
// Subscriptions
planning.get('/subscriptions', async (c) => {
    const householdId = c.get('householdId');
    const { results } = await c.env.DB.prepare('SELECT * FROM subscriptions WHERE household_id = ?').bind(householdId).all();
...
planning.post('/subscriptions', zValidator('json', SubscriptionSchema), async (c) => {
    ...
    await c.env.DB.prepare(`INSERT INTO subscriptions (id, household_id, name, amount_cents, billing_cycle, next_billing_date, account_id, payment_mode, owner_id) 
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, householdId, name, amount_cents, billing_cycle, next_billing_date, account_id || null, payment_mode || 'manual', owner_id || c.get('userId')).run();
...
planning.patch('/subscriptions/:id', zValidator('json', SubscriptionSchema.partial()), async (c) => {
    ...
    await c.env.DB.prepare(`UPDATE subscriptions SET name = COALESCE(?, name), amount_cents = COALESCE(?, amount_cents), billing_cycle = COALESCE(?, billing_cycle), 
     next_billing_date = COALESCE(?, next_billing_date), account_id = COALESCE(?, account_id), payment_mode = COALESCE(?, payment_mode)
     WHERE id = ? AND household_id = ?`).bind(name, amount_cents, billing_cycle, next_billing_date, account_id, payment_mode, id, householdId).run();
...
planning.post('/subscriptions/:id/transfer', zValidator('json', OwnershipTransferSchema), async (c) => {
    ...
    if (transfer_history) {
        queries.push(c.env.DB.prepare('UPDATE transactions SET owner_id = ? WHERE household_id = ? AND description LIKE ?')
            .bind(new_owner_id, householdId, `%${sub.name}%`));
    }
    await c.env.DB.batch(queries);
    await logAudit(c, 'subscriptions', id, 'TRANSFER_OWNERSHIP', { old_owner: sub.owner_id }, { new_owner_id, transfer_history });
    return c.json({ success: true });
});
// Installment Plans
...
planning.post('/installment-plans', zValidator('json', InstallmentPlanSchema), async (c) => {
    ...
    const id = crypto.randomUUID();
    await c.env.DB.prepare(`INSERT INTO installment_plans (id, household_id, name, total_amount_cents, installment_amount_cents, total_installments, remaining_installments, frequency, next_payment_date, account_id, payment_mode) 
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, householdId, name, total_amount_cents, installment_amount_cents, total_installments, total_installments, frequency, next_payment_date, account_id || null, payment_mode || 'manual').run();
...
planning.patch('/installment-plans/:id', zValidator('json', InstallmentPlanSchema.partial()), async (c) => {
    ...
    if (!old)
        return c.json({ error: 'Not found' }, 404);
    const { name, total_amount_cents, installment_amount_cents, total_installments, remaining_installments, next_payment_date, account_id, payment_mode, status } = data;
    await c.env.DB.prepare(`UPDATE installment_plans SET 
     ...
     WHERE id = ? AND household_id = ?`).bind(name, total_amount_cents, installment_amount_cents, total_installments, remaining_installments, next_payment_date, account_id, payment_mode, status, id, householdId).run();
...
planning.post('/p2p/loans', zValidator('json', LoanSchema), async (c) => {
    ...
    const data = c.req.valid('json');
    const id = crypto.randomUUID();
    await c.env.DB.prepare('INSERT INTO personal_loans (id, household_id, lender_user_id, borrower_name, borrower_contact, total_amount_cents, remaining_balance_cents, interest_rate_apy, term_months, origination_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(id, householdId, userId, data.borrower_name, data.borrower_contact, data.total_amount_cents, data.total_amount_cents, data.interest_rate_apy || 0, data.term_months, data.origination_date).run();
    return c.json({ success: true, id });
});
planning.post('/p2p/loans/:id/payments', zValidator('json', z.object({
    ...
    // 1. Log Payment
    await c.env.DB.prepare('INSERT INTO loan_payments (id, loan_id, amount_cents, platform, external_id, method) VALUES (?, ?, ?, ?, ?, ?)').bind(id, loanId, data.amount_cents, data.platform, data.external_id, data.method).run();
    // 2. Update Balance
    await c.env.DB.prepare('UPDATE personal_loans SET remaining_balance_cents = remaining_balance_cents - ? WHERE id = ?').bind(data.amount_cents, loanId).run();
...
planning.get('/bills/upcoming', async (c) => {
    ...
    return c.json({
        ...
        total_upcoming_cents: [...subs, ...installments].reduce((acc, curr) => acc + (curr.amount_cents || curr.installment_amount_cents || 0), 0)
    });
});
// Budgets & Envelopes
planning.get('/budgets', async (c) => {
    const householdId = c.get('householdId');
    const household = await c.env.DB.prepare('SELECT unallocated_balance_cents FROM households WHERE id = ?').bind(householdId).first();
    ...
    const budgets = categories.map((cat) => {
        const spend = spends.find((s) => s.category_id === cat.id)?.total_spend || 0;
        return {
            ...cat,
            spend_cents: spend
        };
...
planning.post('/budget/rollover', async (c) => {
    ...
    const { results: categories } = await c.env.DB.prepare('SELECT id, rollover_enabled, monthly_budget_cents FROM categories WHERE household_id = ?').bind(householdId).all();
    if (!categories || categories.length === 0)
        return c.json({ success: true, count: 0 });
    const queries = categories.map((cat) => {
        if (cat.rollover_enabled) {
            // Add budget to existing balance
            return c.env.DB.prepare('UPDATE categories SET envelope_balance_cents = envelope_balance_cents + ? WHERE id = ?').bind(cat.monthly_budget_cents, cat.id);
        }
        else {
            // Reset to budget amount (clear remainder)
            return c.env.DB.prepare('UPDATE categories SET envelope_balance_cents = ? WHERE id = ?').bind(cat.monthly_budget_cents, cat.id);
        }
    });
    await c.env.DB.batch(queries);
...
planning.get('/templates', async (c) => {
    const householdId = c.get('householdId');
    const { results } = await c.env.DB.prepare('SELECT * FROM templates WHERE household_id = ?').bind(householdId).all();
    return c.json(results);
});

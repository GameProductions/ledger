...
discord.post('/interactions', async (c) => {
    ...
    if (interaction.type === InteractionType.APPLICATION_COMMAND) {
        ...
        if (name === 'ledger-upcoming') {
            ...
            if (subs.length === 0)
                content += "No bills due soon. You're all clear!";
            else
                subs.forEach(s => { content += `- ${s.name}: **$${(s.amount_cents / 100).toFixed(2)}** on ${s.next_billing_date}\n`; });
        ...
        if (name === 'ledger-forecast' || name === 'ledger-report') {
            const { results: categories } = await c.env.DB.prepare('SELECT name, envelope_balance_cents FROM categories WHERE household_id = ? LIMIT 5').bind(householdId).all();
            const labels = categories.map(cat => cat.name);
            const data = categories.map(cat => cat.envelope_balance_cents / 100);
        ...
        if (name === 'ledger-audit') {
            ...
            results.forEach(r => { content += `- ${r.created_at}: **${r.action}** on \`${r.table_name}\`\n`; });
            return c.json({ type: InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE, data: { content } });
        }
    }

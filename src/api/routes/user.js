...
import { ProfileSchema, CreateHouseholdSchema, UpdateHouseholdSchema, UserPaymentMethodSchema, UserLinkedAccountSchema, JoinHouseholdSchema } from '../schemas';
...
user.get('/profile', async (c) => {
    ...
    const { results } = await c.env.DB.prepare('SELECT id, email, display_name, avatar_url, global_role, status, settings_json, created_at FROM users WHERE id = ?').bind(userId).all();
...
user.get('/onboarding', async (c) => {
    ...
    const { results: stepResults } = await c.env.DB.prepare('SELECT step_id FROM user_onboarding WHERE user_id = ? AND status = "completed"').bind(userId).all();
    const completedSteps = stepResults?.map(r => r.step_id) || [];
    ...
    const lastVersion = user?.last_viewed_version || 'v1.0.0';
    // 3. Filter updates
...
user.post('/onboarding/step', zValidator('json', z.object({
    ...
    // Update step status
    await c.env.DB.prepare('INSERT OR REPLACE INTO user_onboarding (id, user_id, step_id, status, completed_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)').bind(crypto.randomUUID(), userId, step, 'completed').run();
    ...
    const completedSteps = results?.map(r => r.step_id) || [];
    return c.json({
        success: true,
        completed_steps: completedSteps,
...
user.post('/households/invite', zValidator('json', z.object({ email: z.string().email().optional() }).optional()), async (c) => {
    ...
    const household = await c.env.DB.prepare('SELECT h.name, uh.role FROM households h JOIN user_households uh ON h.id = uh.household_id WHERE uh.user_id = ? AND h.id = ?').bind(userId, householdId).first();
    ...
    await c.env.DB.prepare('INSERT INTO household_invites (id, household_id, created_by, expires_at) VALUES (?, ?, ?, ?)').bind(id, householdId, userId, expiresAt.toISOString()).run();
...
user.post('/households/join', zValidator('json', JoinHouseholdSchema), async (c) => {
    ...
    // 1. Validate invite
    const invite = await c.env.DB.prepare('SELECT * FROM household_invites WHERE id = ? AND status = "pending"').bind(token).first();
    if (!invite)
        throw new HTTPException(404, { message: 'Invitation not found or already accepted' });
    ...
    const existing = await c.env.DB.prepare('SELECT role FROM user_households WHERE user_id = ? AND household_id = ?').bind(userId, invite.household_id).first();
    if (existing)
        throw new HTTPException(409, { message: 'You are already a member of this household' });
    // 3. Join
    await c.env.DB.batch([
...
user.patch('/households/:id', zValidator('json', UpdateHouseholdSchema), async (c) => {
    ...
    if (globalRole !== 'super_admin') {
        const membership = await c.env.DB.prepare('SELECT role FROM user_households WHERE user_id = ? AND household_id = ?').bind(c.get('userId'), id).first();
...
user.get('/preferences', async (c) => {
    const userId = c.get('userId');
    const { results } = await c.env.DB.prepare('SELECT key, value FROM user_preferences WHERE user_id = ?').bind(userId).all();
...
user.patch('/preferences', zValidator('json', z.record(z.string(), z.string())), async (c) => {
    ...
    const queries = Object.entries(prefs).map(([key, value]) => c.env.DB.prepare('INSERT OR REPLACE INTO user_preferences (user_id, key, value) VALUES (?, ?, ?)')
...
user.get('/notifications', async (c) => {
    const userId = c.get('userId');
    const { results } = await c.env.DB.prepare('SELECT * FROM notification_settings WHERE user_id = ?').bind(userId).all();
    return c.json(results);
});
user.patch('/notifications', zValidator('json', z.array(z.object({
    ...
    const queries = settings.map(s => c.env.DB.prepare('INSERT OR REPLACE INTO notification_settings (user_id, type, event, enabled, offset_days) VALUES (?, ?, ?, ?, ?)')
...
user.post('/payment-methods', zValidator('json', UserPaymentMethodSchema), async (c) => {
    ...
    const data = c.req.valid('json');
    const id = crypto.randomUUID();
    await c.env.DB.prepare('INSERT INTO user_payment_methods (id, user_id, household_id, name, type, last_four, branding_url) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(id, userId, householdId, data.name, data.type, data.last_four || null, data.branding_url || null).run();
...
user.get('/providers', async (c) => {
    const userId = c.get('userId');
    const { results } = await c.env.DB.prepare(`SELECT lp.*, sp.name as provider_name, sp.icon_url 
     FROM linked_providers lp
     JOIN service_providers sp ON lp.service_provider_id = sp.id
     WHERE lp.user_id = ?`).bind(userId).all();
    return c.json(results);
});
user.post('/providers/link', zValidator('json', z.object({
    ...
    const id = crypto.randomUUID();
    await c.env.DB.prepare('INSERT INTO linked_providers (id, user_id, service_provider_id, account_reference, custom_label, metadata) VALUES (?, ?, ?, ?, ?, ?)').bind(id, userId, serviceProviderId, accountReference || null, customLabel || null, metadata || null).run();
    return c.json({ success: true, id });
});
...
user.post('/linked-accounts', zValidator('json', UserLinkedAccountSchema), async (c) => {
    ...
    const data = c.req.valid('json');
    const id = crypto.randomUUID();
    await c.env.DB.prepare(`INSERT INTO user_linked_accounts (id, user_id, household_id, provider_id, payment_method_id, email_attached, membership_start_date, membership_end_date, subscription_id, notes, status) 
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, userId, householdId, data.provider_id, data.payment_method_id, data.email_attached, data.membership_start_date, data.membership_end_date, data.subscription_id, data.notes, data.status).run();
...
user.get('/passkeys', async (c) => {
    const userId = c.get('userId');
    const { results } = await c.env.DB.prepare('SELECT id, name, aaguid, created_at FROM passkeys WHERE user_id = ?').bind(userId).all();
    return c.json(results);
});
export default user;

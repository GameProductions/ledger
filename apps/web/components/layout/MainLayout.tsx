  ...
  return (
    <div className={`min-h-screen flex flex-col transition-all duration-700 ${isLuxury ? 'bg-luxury-grid' : ''}`}>
      ...
      <main className={`flex-1 w-full transition-all duration-700 ${
        isLuxury ? 'max-w-5xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8 pt-4 sm:pt-40 pb-28 sm:pb-24' : 
        isBusiness ? 'max-w-[100vw] px-2 sm:px-4 md:px-6 pt-4 sm:pt-28 pb-20 sm:pb-12 flex flex-col sm:flex-row sm:space-x-8' : 
        'max-w-7xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8 pt-4 sm:pt-32 pb-24 sm:pb-16'
      }`}>
        {isBusiness && (
          <aside className="hidden lg:block w-64 flex-shrink-0 card p-4 h-[calc(100vh-8rem)] sticky top-28 bg-deep/40 backdrop-blur-xl border-glass-border">
            <nav className="space-y-2">
              <div className="text-xs tracking-widest text-secondary font-bold mb-4 opacity-50 px-2">Focus Navigation</div>

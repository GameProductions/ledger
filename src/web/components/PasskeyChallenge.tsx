...
export function PasskeyChallenge({ onSuccess, appName = 'System' }: PasskeyChallengeProps) {
  const { token } = useAuth();
  ...
  const initiateChallenge = async () => {
    ...
    try {
      if (mode === 'auth') {
        const resp = await fetch(`${API_URL}/api/auth/webauthn/generate-auth`, { 
          method: 'POST',
        ...
        const verificationResp = await fetch(`${API_URL}/api/auth/webauthn/verify-auth`, {
          method: 'POST',
        ...
        if (verificationResp.ok) {
          ...
          const { error } = await verificationResp.json() as any;
          setError(error || 'Verification failed');
        }
      } else {
        const resp = await fetch(`${API_URL}/api/auth/webauthn/generate-registration`, { 
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const options = await resp.json() as any;
        ...
        const verificationResp = await fetch(`${API_URL}/api/auth/webauthn/verify-registration`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-8 max-w-sm w-full mx-4 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative flex flex-col items-center text-center">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-8 max-w-[260px] leading-relaxed">
            {mode === 'auth' 
              ? 'Verify your identity using a cryptographic passkey to access God Mode.' 

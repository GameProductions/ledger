...
support.post('/issues', zValidator('json', SupportIssueSchema), async (c) => {
    ...
    `).run();
        ...
        await c.env.DB.prepare('INSERT INTO support_issues (id, user_id, title, description, category, priority) VALUES (?, ?, ?, ?, ?, ?)').bind(id, userId, title, description, category || 'General', priority).run();

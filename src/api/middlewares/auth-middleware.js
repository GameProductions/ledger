...
export const authMiddleware = async (c, next) => {
    try {
        ...
        if (token.startsWith('ledger_')) {
            const { results } = await c.env.DB.prepare('SELECT household_id FROM personal_access_tokens WHERE id = ?').bind(token).all();
            if (results.length > 0) {
        ...
        const user = await c.env.DB.prepare('SELECT id, global_role, status FROM users WHERE id = ?').bind(payload.sub).first();
        if (!user) {
        ...
        c.executionCtx.waitUntil(c.env.DB.prepare('UPDATE users SET last_active_at = CURRENT_TIMESTAMP WHERE id = ?')
            .bind(userId).run());
        ...
        const householdExists = await c.env.DB.prepare('SELECT id FROM households WHERE id = ?').bind(activeHouseholdId).first();
        if (!householdExists) {
        ...
        if (globalRole !== 'super_admin') {
            const dbRes = await c.env.DB.prepare('SELECT role FROM user_households WHERE user_id = ? AND household_id = ?').bind(userId, activeHouseholdId).first();
            if (!dbRes) {
        ...
        if (payload.impersonatorId)
            c.set('impersonatorId', payload.impersonatorId);
        await next();
    }
    catch (e) {

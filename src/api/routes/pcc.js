...
import { z } from 'zod';
import { BillingProcessorSchema, ProviderSchema, UpdateSystemConfigSchema, UpdateSystemFeatureSchema, SystemRegistrySchema, UpdateUserAdminSchema, CreateUserAdminSchema } from '../schemas';
...
pcc.get('/stats', async (c) => {
    ...
    return c.json({
        ...
        totalHouseholds: householdCount?.[0]?.count || 0,
...
pcc.patch('/features/:id', zValidator('json', UpdateSystemFeatureSchema), async (c) => {
    ...
    await c.env.DB.prepare('UPDATE system_feature_flags SET enabled_globally = ?, target_user_ids = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').bind(enabled_globally ? 1 : 0, target_user_ids, id).run();
...
pcc.post('/records', zValidator('json', SystemRegistrySchema), async (c) => {
    ...
    await c.env.DB.prepare('INSERT INTO system_registry (id, item_type, name, logo_url, website_url, metadata_json) VALUES (?, ?, ?, ?, ?, ?)').bind(id, data.item_type, data.name, data.logo_url || null, data.website_url || null, JSON.stringify(metadata)).run();
...
pcc.post('/admin/users', zValidator('json', CreateUserAdminSchema, (result, c) => {
    ...
    await c.env.DB.prepare('INSERT INTO users (id, username, email, password_hash, display_name, global_role, status, force_password_change) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(userId, username, email, passwordHash, display_name, global_role, 'active', force_password_change ? 1 : 0).run();
...
pcc.get('/search', async (c) => {
    ...
    const { results: users } = await c.env.DB.prepare('SELECT id, email, display_name FROM users WHERE email LIKE ? OR display_name LIKE ? LIMIT 10').bind(`%${q}%`, `%${q}%`).all();
    const { results: registry } = await c.env.DB.prepare('SELECT id, name, item_type FROM system_registry WHERE name LIKE ? LIMIT 10').bind(`%${q}%`).all();
    return c.json({ users, registry });
});
...
pcc.post('/networks', zValidator('json', BillingProcessorSchema), async (c) => {
    const data = c.req.valid('json');
    const id = crypto.randomUUID();
    await c.env.DB.prepare('INSERT INTO billing_processors (id, name, website_url, branding_url, support_url, subscription_id_notes) VALUES (?, ?, ?, ?, ?, ?)').bind(id, data.name, data.website_url, data.branding_url, data.support_url, data.subscription_id_notes).run();
...
pcc.patch('/networks/:id', zValidator('json', BillingProcessorSchema.partial()), async (c) => {
    ...
    await c.env.DB.prepare(`UPDATE billing_processors SET 
     ...
     WHERE id = ?`).bind(name, website_url, branding_url, support_url, subscription_id_notes, id).run();
...
pcc.post('/providers', zValidator('json', ProviderSchema), async (c) => {
    const data = c.req.valid('json');
    const id = crypto.randomUUID();
    await c.env.DB.prepare('INSERT INTO service_providers (id, name, url, icon_url, billing_processor_id, is_3rd_party_capable) VALUES (?, ?, ?, ?, ?, ?)').bind(id, data.name, data.website_url || null, data.branding_url || null, data.billing_processor_id, data.is_3rd_party_capable ? 1 : 0).run();
...
pcc.patch('/providers/:id', zValidator('json', ProviderSchema.partial()), async (c) => {
    ...
    try {
        await c.env.DB.prepare(`UPDATE service_providers SET 
       ...
       WHERE id = ?`).bind(name, website_url, branding_url, billing_processor_id, is_3rd_party_capable !== undefined ? (is_3rd_party_capable ? 1 : 0) : null, id).run();
...
pcc.post('/walkthroughs', zValidator('json', z.object({
    ...
    const id = crypto.randomUUID();
    await c.env.DB.prepare('INSERT INTO system_walkthroughs (id, version, title, content_md, created_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)').bind(id, version, title, content_md).run();
...
pcc.get('/admin/users/:userId/details', async (c) => {
  ...
  `).bind(userId).first();
    if (!user)
        throw new HTTPException(404, { message: 'User not found' });
    const { results: connections } = await c.env.DB.prepare('SELECT id, provider, status, created_at FROM external_connections WHERE household_id IN (SELECT household_id FROM user_households WHERE user_id = ?)').bind(userId).all();
    const { results: socialLinks } = await c.env.DB.prepare('SELECT id, provider, provider_user_id as provider_id, created_at FROM user_identities WHERE user_id = ?').bind(userId).all();
    const { results: passkeys } = await c.env.DB.prepare('SELECT id, name, aaguid, created_at FROM passkeys WHERE user_id = ?').bind(userId).all();
...
pcc.post('/admin/users/merge', zValidator('json', z.object({
    ...
    const source = await c.env.DB.prepare('SELECT id, display_name, email FROM users WHERE id = ?').bind(sourceId).first();
    const target = await c.env.DB.prepare('SELECT id, display_name, email FROM users WHERE id = ?').bind(targetId).first();
    if (!source || !target) {
    ...
    await c.env.DB.batch([
        ...
        c.env.DB.prepare('INSERT OR IGNORE INTO user_households (user_id, household_id) SELECT ? as user_id, household_id FROM user_households WHERE user_id = ?').bind(targetId, sourceId),
    ...
    await logAudit(c, 'users', targetId, 'ADMIN_USER_MERGE', { merged_source_id: sourceId, source_email: source.email }, { source_name: source.display_name, action: 'UNIFICATION_COMPLETE' });
    return c.json({ success: true });
});
// System Announcements (v3.21)
pcc.get('/announcements', async (c) => {
    const { results } = await c.env.DB.prepare('SELECT * FROM system_announcements WHERE is_active = 1 AND (expires_at IS NULL OR expires_at > CURRENT_TIMESTAMP) ORDER BY created_at DESC').all();
    return c.json(results);
});
pcc.post('/announcements', zValidator('json', z.object({
    ...
    await c.env.DB.prepare('INSERT INTO system_announcements (id, title, content_md, priority, actor_id, expires_at) VALUES (?, ?, ?, ?, ?, ?)').bind(id, title, content_md, priority, actorId, expiresAt).run();
...
pcc.post('/admin/users/:userId/impersonate', async (c) => {
    try {
        ...
        const { results: households } = await c.env.DB.prepare('SELECT household_id FROM user_households WHERE user_id = ? LIMIT 1').bind(userId).all();
        const targetHouseholdId = households?.[0]?.household_id || 'ledger-main-001';
        const adminId = c.get('userId');
        const authService = new AuthService(c.env);
        const token = await authService.generateToken(userId, targetHouseholdId, adminId);
        await logAudit(c, 'users', userId, 'ADMIN_IMPERSONATE_START', { actor_id: adminId, target_email: target.email }, {

...
export class AuthService {
    ...
    async validateCredentials(identifier, password) {
        ...
        const user = await this.env.DB.prepare('SELECT * FROM users WHERE username = ? OR email = ?').bind(identifier, identifier).first();
        if (!user) {
    ...
    async generateToken(userId, householdId = 'ledger-main-001', impersonatorId) {
        const payload = {
            sub: userId,
            householdId,
    ...
    async findOrCreateSocialUser(provider, profile, tokens) {
        let identity = await this.env.DB.prepare('SELECT user_id FROM user_identities WHERE provider = ? AND provider_user_id = ?').bind(provider, profile.id).first();
        let userId = identity?.user_id;
        if (!userId) {
            const user = await this.env.DB.prepare('SELECT id FROM users WHERE email = ?').bind(profile.email).first();
            if (user) {
                userId = user.id;
            }
            else {
    ...
    async linkSocialAccount(userId, provider, profile, tokens) {
        ...
        const existing = await this.env.DB.prepare('SELECT user_id FROM user_identities WHERE provider = ? AND provider_user_id = ?').bind(provider, profile.id).first();
    ...
    async verifyAndEnableTOTP(userId, code) {
        const user = await this.env.DB.prepare('SELECT totp_secret FROM users WHERE id = ?').bind(userId).first();
    ...
    async createAdminInvite(role = 'super_admin') {
        ...
        await this.env.DB.prepare('INSERT INTO admin_invitations (token, role, expires_at) VALUES (?, ?, ?)').bind(token, role, expiresAt.toISOString()).run();
        return token;
    }
    async consumeAdminInvite(token, username, password, email) {
        // 1. Verify invitation
        const invite = await this.env.DB.prepare('SELECT * FROM admin_invitations WHERE token = ? AND is_claimed = 0 AND expires_at > CURRENT_TIMESTAMP').bind(token).first();
        if (!invite)
        ...
        await this.env.DB.batch([
            this.env.DB.prepare('INSERT INTO users (id, username, email, password_hash, global_role, status) VALUES (?, ?, ?, ?, ?, ?)').bind(userId, username, email, passwordHash, invite.role, 'active'),
            this.env.DB.prepare('UPDATE admin_invitations SET is_claimed = 1 WHERE token = ?').bind(token)
        ]);
        return userId;
    }
    // --- PASSWORD LIFECYCLE (v2.4.0) ---
    async requestPasswordReset(identifier) {
        const user = await this.env.DB.prepare('SELECT id, email FROM users WHERE email = ? OR username = ?').bind(identifier, identifier).first();
        if (!user)
            return null; // Silent fail for security
        const token = crypto.randomUUID();
        const expiresAt = new Date(Date.now() + 3600000).toISOString(); // 1h
        await this.env.DB.prepare('INSERT INTO password_resets (id, user_id, token, expires_at) VALUES (?, ?, ?, ?)').bind(crypto.randomUUID(), user.id, token, expiresAt).run();
    ...
    async resetPassword(token, newPassword) {
        ...
        const reset = await this.env.DB.prepare('SELECT * FROM password_resets WHERE token = ? AND is_used = 0 AND expires_at > DATETIME("now")').bind(token).first();
        if (!reset)
    ...
    async adminResetPassword(userId, newPassword, isTemporary = false) {
        const passwordHash = await hashPassword(newPassword);
    ...
    async updatePasskeyName(passkeyId, userId, name, isAdmin = false) {
        const query = isAdmin
    ...
    async removePasskey(passkeyId, userId, isAdmin = false) {
        const query = isAdmin

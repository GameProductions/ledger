...
export function PasskeyChallenge({ onSuccess, appName, children }: PasskeyChallengeProps) {
  ...
  const initiateChallenge = async () => {
    ...
    try {
      if (mode === 'auth') {
        const resp = await fetch(`${getApiUrl()}/api/admin/webauthn/generate-auth`, { method: 'POST' });
        if (resp.status === 404) {
        ...
        const verificationResp = await fetch(`${getApiUrl()}/api/admin/webauthn/verify-auth`, {
          method: 'POST',
        ...
        const resp = await fetch(`${getApiUrl()}/api/admin/webauthn/generate-registration`, { method: 'POST' });
        if (!resp.ok) {
          const data = await resp.json().catch(() => ({ error: `Server error ${resp.status}` }));
        ...
        const verificationResp = await fetch(`${getApiUrl()}/api/admin/webauthn/verify-registration`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },

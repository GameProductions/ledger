...
import { TransactionSchema, PaginationSchema, TransferSchema, CreditCardSchema, TimelineEntrySchema } from '../schemas';
import { dispatchWebhook } from '../services/webhook-service';
const financials = new Hono();
// Categories
financials.get('/categories', async (c) => {
    const householdId = c.get('householdId');
    const { results } = await c.env.DB.prepare('SELECT * FROM categories WHERE household_id = ?').bind(householdId).all();
    return c.json(results);
});
// Accounts
financials.get('/accounts', async (c) => {
    const householdId = c.get('householdId');
    const { results } = await c.env.DB.prepare('SELECT id, name, type, balance_cents, currency FROM accounts WHERE household_id = ?').bind(householdId).all();
    return c.json(results);
});
// Credit Cards
...
financials.post('/credit-cards', zValidator('json', CreditCardSchema), async (c) => {
    ...
    await c.env.DB.prepare('INSERT INTO credit_cards (id, household_id, account_id, credit_limit_cents, interest_rate_apy, statement_closing_day, payment_due_day) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(id, householdId, data.account_id, data.credit_limit_cents, data.interest_rate_apy || 0, data.statement_closing_day, data.payment_due_day).run();
...
financials.post('/transactions', zValidator('json', TransactionSchema), async (c) => {
    ...
    const query = c.env.DB.prepare('INSERT INTO transactions (id, household_id, account_id, category_id, description, amount_cents, transaction_date) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(id, householdId, data.account_id, data.category_id, data.description, data.amount_cents, date);
    if (data.category_id) {
...
financials.get('/transactions/export', async (c) => {
    ...
    const { results } = await c.env.DB.prepare(`SELECT t.*, c.name as category_name, a.name as account_name 
     FROM transactions t
     LEFT JOIN categories c ON t.category_id = c.id
     LEFT JOIN accounts a ON t.account_id = a.id
     WHERE t.household_id = ?`).bind(householdId).all();
    if (format === 'csv') {
        if (results.length === 0)
            return c.text('');
...
financials.get('/transactions/:id/timeline', async (c) => {
    ...
    const { results } = await c.env.DB.prepare('SELECT * FROM transaction_timeline WHERE transaction_id = ? ORDER BY created_at DESC').bind(id).all();
    return c.json(results);
});
financials.post('/transactions/:id/timeline', zValidator('json', TimelineEntrySchema), async (c) => {
    ...
    await c.env.DB.prepare('INSERT INTO transaction_timeline (id, transaction_id, type, content) VALUES (?, ?, ?, ?)').bind(entryId, id, type, content).run();
...
financials.post('/transactions/:id/unlink', async (c) => {
    ...
    if (tx && tx.linked_transaction_id) {
        const targetId = tx.linked_transaction_id;
...
financials.patch('/transactions/:id', zValidator('json', TransactionSchema.partial()), async (c) => {
    ...
    if (updates.length > 0) {
        ...
        await c.env.DB.prepare(`UPDATE transactions SET ${updates.join(', ')} WHERE id = ? AND household_id = ?`).bind(...params).run();
        if (data.status) {
            await c.env.DB.prepare('INSERT INTO transaction_timeline (id, transaction_id, type, content) VALUES (?, ?, ?, ?)').bind(crypto.randomUUID(), id, 'status_change', `Status changed to ${data.status}`).run();
        }
    }
    return c.json({ success: true });
});
// Transaction Import Analysis
financials.post('/transactions/import/analyze', async (c) => {
...
financials.post('/transactions/import/confirm', zValidator('json', z.object({
    ...
    const queries = data.map(row => {
        ...
        return c.env.DB.prepare('INSERT INTO transactions (id, household_id, account_id, description, amount_cents, transaction_date) VALUES (?, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), householdId, accountId, description, amountCents, date);
    });
...
financials.post('/transactions/:id/receipt', async (c) => {
    ...
    if (!file)
        return c.json({ error: 'No file uploaded' }, 400);
    if (!c.env.ASSETS)
        return c.json({ error: 'Assets Bucket not configured' }, 500);
    const key = `receipts/${householdId}/${id}-${Date.now()}`;
    await c.env.ASSETS.put(key, await file.arrayBuffer(), {
        httpMetadata: { contentType: file.type }
    });
    await c.env.DB.prepare('UPDATE transactions SET receipt_r2_key = ? WHERE id = ? AND household_id = ?').bind(key, id, householdId).run();
...
financials.get('/transactions/:id/receipt', async (c) => {
    ...
    if (!tx || !tx.receipt_r2_key)
        return c.json({ error: 'Receipt not found' }, 404);
    const object = await c.env.ASSETS.get(tx.receipt_r2_key);
    if (!object)
...
financials.post('/transfers', zValidator('json', TransferSchema), async (c) => {
    ...
    if (verify.results.length < 2)
        throw new HTTPException(403, { message: 'One or more accounts unauthorized' });
    const id = crypto.randomUUID();
    await c.env.DB.prepare('UPDATE accounts SET balance_cents = balance_cents - ? WHERE id = ? AND household_id = ?').bind(amount_cents, from_account_id, householdId).run();
    await c.env.DB.prepare('UPDATE accounts SET balance_cents = balance_cents + ? WHERE id = ? AND household_id = ?').bind(amount_cents, to_account_id, householdId).run();

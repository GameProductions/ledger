...
import SharedBalances from '../components/SharedBalances'
...
const DEFAULT_LAYOUT: Record<string, { id: string, visible: boolean }[]> = {
  ...
  planning: [
    ...
    { id: 'shared-balances', visible: true }
  ],
  insights: [
                    ...
                    <div key={b.id} className="interactive-card hover:border-primary/30 transition-all group p-4 border-l-4 border-l-transparent hover:border-l-primary">
                       ...
                       <div className="space-y-2">
                          ...
                          <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                             <div 
                               className={`h-full transition-all duration-700 ${((b.spend_cents || 0) > (b.monthlyBudgetCents || 0)) ? 'bg-red-500' : 'bg-primary'}`}
                               style={{ width: `${Math.min(100, Math.max(0, ((b.spend_cents || 0) / (b.monthlyBudgetCents || 1)) * 100))}%` }}
                             />
                          </div>
                       </div>
                    </div>

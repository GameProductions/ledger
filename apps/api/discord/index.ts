...
discord.post('/interactions', async (c) => {
  ...
  const isValidRequest = (await verifyKey(body, signature || '', timestamp || '', c.env.DISCORD_PUBLIC_KEY) as any)
  ...
  if (interaction.type === InteractionType.APPLICATION_COMMAND) {
    ...
    if (name === 'ledger-upcoming') {
      ...
      const subs = (await db.select({
            ...
            }).from(subscriptions).where(and(eq(subscriptions.householdId, householdId), lte(subscriptions.nextBillingDate, endDateStr))) as any);

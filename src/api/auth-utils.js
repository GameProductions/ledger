...
export async function generateTOTPToken(secret, counter) {
...
export async function verifyWebAuthnAssertion(publicKey, signature, clientDataJSON, authenticatorData, challenge) {
    ...
    try {
        const key = await crypto.subtle.importKey('spki', publicKey, { name: 'ECDSA', namedCurve: 'P-256', hash: 'SHA-256' }, false, ['verify']);
...
export async function verifyWebAuthnRegistration(attestationResponse, challenge) {
...
export async function verifyPassword(password, storedHash) {
    try {
        ...
        if (!isMatched)
            console.warn('[Auth] Password hash mismatch');
        return isMatched;
    }
    catch (e) {

...
ledger.get('/api/config', async (c) => {
    ...
    return c.json(configs.reduce((acc, curr) => ({ ...acc, [curr.config_key]: JSON.parse(curr.config_value) }), {}));
});
ledger.get('/api/theme/broadcast', async (c) => {
    try {
        ...
        return c.json({ themeId: JSON.parse(config.config_value) });
    }
    catch (e) {
        return c.json({ themeId: null });
    }

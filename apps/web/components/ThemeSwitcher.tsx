      ...
      {categories.map(category => (
        <div key={category} className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-black tracking-[0.2em] text-secondary opacity-60">
              {category} Range
            </span>
            <div className="h-px flex-1 bg-glass-border" />
          </div>
            ...
            {ThemeRegistry.filter(t => t.category === category).map(t => (
              ...
              >
                <div className="flex justify-between items-start z-10">
                  <span className={`text-xs font-black tracking-widest ${activeTheme.id === t.id ? 'text-primary' : 'text-secondary'}`}>
                    {t.name}
                  </span>
                  {activeTheme.id === t.id && (
